...
async function loadReconciliations(page = 1) {
  currentPage = page;
  try {
    const result = await fetchAPI(`reconciliation?page=${page}&limit=${itemsPerPage}`);
    if (result.success) {
      ...
      if (reconciliations.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 2rem;">لا توجد تسويات</td></tr>';
      } else {
        reconciliations.forEach(rec => {
          const row = document.createElement('tr');
          const diff = parseFloat(rec.difference || 0);
          const diffClass = Math.abs(diff) < 0.01 ? 'balance-positive' : 'balance-negative';
          ...
          row.innerHTML = `
            ...
            <td>${rec.notes || ''}</td>
            <td>
              <button class="btn-icon" onclick="viewReconciliation(${rec.id})" title="عرض">
      ...
      renderPagination('reconciliation-pagination', currentPage, Math.ceil(total / itemsPerPage), loadReconciliations);
    } else {
      showAlert('alert-container', result.message || 'فشل تحميل التسويات', 'error');
    }
  } catch (error) {
...
function showCreateModal() {
  ...
  document.getElementById('reconciliation-modal').style.display = 'flex';
}
...
async function calculateReconciliation() {
  ...
  try {
    ...
    document.getElementById('bank-balance-display').textContent = formatCurrency(bankBalance);
...
async function createAdjustment(reconciliationId) {
  ...
  const entryType = confirm('هل هذا مبلغ مدين؟ (نعم = مدين، لا = دائن)') ? 'DEBIT' : 'CREDIT';
  ...
  try {
    ...
    if (result.success) {
      ...
      showAlert('alert-container', result.message || 'فشل إنشاء قيد التسوية', 'error');
    }
  } catch (error) {
...
document.getElementById('reconciliation-form').addEventListener('submit', async function(e) {
  ...
  const data = {
    ...
    bank_balance: parseFloat(document.getElementById('bank-balance').value || 0),
  ...
  try {
    ...
    if (result.success) {
      ...
      showAlert('alert-container', result.message || 'فشل حفظ التسوية', 'error');
    }
  } catch (error) {
