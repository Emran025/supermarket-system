...
async function loadJournalVouchers(page = 1) {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (jvs.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="7" style="text-align: center; padding: 2rem;">لا توجد سندات قيد</td></tr>';
      } else {
        jvs.forEach((jv) => {
          ...
          row.innerHTML = `
            ...
            <td>
              ...
              <button class="btn-icon text-danger" onclick="deleteJV(${
                jv.id
              })" title="حذف">
      ...
      showAlert(
        "alert-container",
        result.message || "فشل تحميل السندات",
        "error"
      );
    }
  } catch (error) {
...
function showCreateModal() {
  ...
  document.getElementById("jv-date").value = new Date()
    .toISOString()
    .split("T")[0];
...
function addEntryRow() {
  ...
  row.innerHTML = `
    ...
    <button type="button" class="icon-btn delete" onclick="removeEntryRow(this)">
      <i class="fas fa-trash"></i>
    </button>
  `;
  ...
  try {
    ...
    if (result.success) {
      ...
      document.getElementById(
        "modal-title"
      ).textContent = `عرض السند: ${jv.voucher_number}`;
      ...
      document
        .getElementById("jv-form")
        .querySelector('button[type="submit"]').style.display = "none";
    }
  } catch (error) {
...
async function editJV(id) {
  ...
  document
    .getElementById("jv-form")
    .querySelector('button[type="submit"]').style.display = "block";
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    if (Math.abs(totalDebit - totalCredit) > 0.01) {
      showAlert(
        "alert-container",
        "يجب أن يتساوى إجمالي المدين والدائن",
        "warning"
      );
      return;
    }
