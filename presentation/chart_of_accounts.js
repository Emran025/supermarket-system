...
async function loadAccounts() {
  try {
    ...
    if (result.success) {
      ...
      showAlert('alert-container', result.message || 'فشل تحميل الحسابات', 'error');
    }
  } catch (error) {
...
function renderAccounts() {
  ...
  if (accounts.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 2rem;">لا توجد حسابات</td></tr>';
  } else {
    accounts.forEach(account => {
      ...
      row.innerHTML = `
        ...
        <td><span class="badge ${account.is_active ? 'badge-success' : 'badge-danger'}">${account.is_active ? 'نشط' : 'غير نشط'}</span></td>
            <td>
              <div class="action-buttons">
                <button class="icon-btn edit" onclick="editAccount(${account.id})" title="تعديل">
                  ${getIcon("edit")}
                </button>
                <button class="icon-btn delete" onclick="deleteAccount(${account.id})" title="حذف">
                  ${getIcon("trash")}
                </button>
              </div>
            </td>
...
async function loadAccountBalances() {
  try {
    ...
    if (result.success) {
      ...
      let content = '<h2>أرصدة الحسابات</h2><table class="data-table"><thead><tr><th>الحساب</th><th>المدين</th><th>الدائن</th><th>الرصيد</th></tr></thead><tbody>';
      ...
      accounts.forEach(acc => {
        if (Math.abs(acc.balance) > 0.01) {
          content += `<tr>
            ...
            <td class="${acc.balance >= 0 ? 'balance-positive' : 'balance-negative'}">${formatCurrency(acc.balance)}</td>
          </tr>`;
        }
      });
...
document.getElementById('account-form').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    if (result.success) {
      ...
      showAlert('alert-container', result.message || 'فشل حفظ الحساب', 'error');
    }
  } catch (error) {
