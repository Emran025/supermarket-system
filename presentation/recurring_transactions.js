...
async function loadRecurringTransactions(page = 1) {
  currentPage = page;
  try {
    const result = await fetchAPI(`recurring_transactions?page=${page}&limit=${itemsPerPage}`);
    if (result.success) {
      ...
      if (templates.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 2rem;">لا توجد قوالب</td></tr>';
      } else {
        templates.forEach(template => {
          ...
          const statusClass = template.next_due_date && new Date(template.next_due_date) <= new Date() ? 'badge-warning' : 'badge-success';
          const statusText = template.next_due_date && new Date(template.next_due_date) <= new Date() ? 'مستحق' : 'نشط';
          
          row.innerHTML = `
            <td><strong>${template.name}</strong></td>
            <td>${typeText}</td>
            <td>${frequencyText}</td>
            <td>${template.next_due_date ? formatDate(template.next_due_date) : '-'}</td>
            <td>${template.last_generated_date ? formatDate(template.last_generated_date) : 'لم ينفذ'}</td>
            <td><span class="badge ${statusClass}">${statusText}</span></td>
            <td>
              <div class="action-buttons">
                <button class="icon-btn view" onclick="viewTemplate(${template.id})" title="عرض">
                  ${getIcon("eye")}
                </button>
                <button class="icon-btn edit" onclick="editTemplate(${template.id})" title="تعديل">
                  ${getIcon("edit")}
                </button>
                <button class="icon-btn delete" onclick="deleteTemplate(${template.id})" title="حذف">
                  ${getIcon("trash")}
                </button>
                <button class="icon-btn edit" onclick="generateTransaction(${template.id})" title="تنفيذ الآن" style="background: var(--success-color); color: white;">
                  ${getIcon("check")}
                </button>
              </div>
            </td>
      ...
      renderPagination('recurring-pagination', currentPage, Math.ceil(total / itemsPerPage), loadRecurringTransactions);
    } else {
      showAlert('alert-container', result.message || 'فشل تحميل القوالب', 'error');
    }
  } catch (error) {
...
function showCreateModal() {
  document.getElementById('modal-title').textContent = 'قالب معاملة متكررة جديد';
  document.getElementById('recurring-form').reset();
  document.getElementById('recurring-id').value = '';
  document.getElementById('next-due-date').value = new Date().toISOString().split('T')[0];
...
function updateTemplateFields() {
  ...
  document.getElementById('revenue-fields').style.display = type === 'revenue' ? 'block' : 'none';
  document.getElementById('journal-fields').style.display = type === 'journal_voucher' ? 'block' : 'none';
  ...
  try {
    ...
    if (result.success) {
      ...
      if (template.type === 'expense') {
        document.getElementById('expense-account').value = templateData.account_code || '';
        document.getElementById('expense-amount').value = templateData.amount || '';
        document.getElementById('expense-description').value = templateData.description || '';
      } else if (template.type === 'revenue') {
        document.getElementById('revenue-account').value = templateData.account_code || '';
        document.getElementById('revenue-amount').value = templateData.amount || '';
        document.getElementById('revenue-description').value = templateData.description || '';
      } else if (template.type === 'journal_voucher') {
        document.getElementById('journal-entries').value = JSON.stringify(templateData.entries || [], null, 2);
      }
...
async function generateTransaction(id) {
  ...
  try {
    const result = await fetchAPI('recurring_transactions?action=process', 'POST', {
      template_id: id,
      generation_date: new Date().toISOString().split('T')[0]
    });
    
    if (result.success) {
      showAlert('alert-container', `تم تنفيذ المعاملة بنجاح. رقم السند: ${result.data?.voucher_number || ''}`, 'success');
      loadRecurringTransactions(currentPage);
    } else {
      showAlert('alert-container', result.message || 'فشل تنفيذ المعاملة', 'error');
    }
  } catch (error) {
...
document.getElementById('recurring-form').addEventListener('submit', async function(e) {
  ...
  if (type === 'expense') {
    ...
    try {
      const entries = JSON.parse(document.getElementById('journal-entries').value);
  ...
  try {
    ...
    if (result.success) {
      ...
      showAlert('alert-container', result.message || 'فشل حفظ القالب', 'error');
    }
  } catch (error) {
