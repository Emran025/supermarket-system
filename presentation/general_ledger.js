...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (dropdownTrigger && glDropdown) {
    dropdownTrigger.addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      glDropdown.classList.toggle("active");
    });

    document.addEventListener("click", function (e) {
      if (
        glDropdown.classList.contains("active") &&
        !glDropdown.contains(e.target)
      ) {
        glDropdown.classList.remove("active");
      }
    });
...
async function loadGLEntries(page = 1) {
  ...
  try {
    ...
    let url = `general_ledger?operation=entries&page=${page}&limit=${itemsPerPage}`;
    ...
    if (result.success) {
      ...
      if (entries.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="8" style="text-align: center; padding: 2rem;">لا توجد قيود</td></tr>';
        ...
        entries.forEach((entry) => {
          ...
          const debit =
            entry.entry_type === "DEBIT" ? parseFloat(entry.amount) : 0;
          const credit =
            entry.entry_type === "CREDIT" ? parseFloat(entry.amount) : 0;
          ...
          row.innerHTML = `
            ...
            <td>
              <div class="action-buttons">
                <button class="icon-btn view" onclick="viewEntry('${
      ...
      showAlert(
        "alert-container",
        result.message || "فشل تحميل القيود",
        "error"
      );
    }
  } catch (error) {
...
async function loadTrialBalance() {
  try {
    ...
    const result = await fetchAPI(
      `general_ledger?operation=trial_balance&as_of_date=${asOfDate}`
    );
    if (result.success) {
      ...
      if (entries.length === 0) {
        body.innerHTML =
          '<div style="padding: 2rem; text-align: center;">لا توجد حسابات</div>';
      } else {
        entries.forEach((entry) => {
          ...
          row.innerHTML = `
            <div><strong>${entry.account_code}</strong> - ${
            entry.account_name
          }</div>
            <div style="text-align: center">${formatCurrency(
              entry.debit_balance || 0
            )}</div>
            <div style="text-align: center">${formatCurrency(
              entry.credit_balance || 0
            )}</div>
            <div style="text-align: center" class="${balanceClass}">${formatCurrency(
            balance
          )}</div>
          `;
      ...
      // Totals
      document.getElementById("tb-total-debits").textContent = formatCurrency(
        summary.total_debits || 0
      );
      document.getElementById("tb-total-credits").textContent = formatCurrency(
        summary.total_credits || 0
      );

      const isBalanced = summary.is_balanced ? "✓ متوازن" : "✗ غير متوازن";
      const balancedClass = summary.is_balanced
        ? "balance-positive"
        : "balance-negative";
      document.getElementById(
        "tb-is-balanced"
      ).innerHTML = `<span class="${balancedClass}">${isBalanced}</span>`;
    } else {
      showAlert(
        "alert-container",
        result.message || "فشل تحميل ميزان المراجعة",
        "error"
      );
    }
  } catch (error) {
...
async function loadAccountHistory() {
  try {
    ...
    let url = `general_ledger?operation=account_details&account_code=${encodeURIComponent(
      accountCode
    )}&page=1&limit=1000`;
    if (startDate) url += `&start_date=${startDate}`;
    if (endDate) url += `&end_date=${endDate}`;
    ...
    if (result.success) {
      ...
      if (history.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align: center; padding: 2rem;">لا توجد معاملات</td></tr>';
      } else {
        history.forEach((item) => {
          const row = document.createElement("tr");
          const debit =
            item.entry_type === "DEBIT" ? parseFloat(item.amount || 0) : 0;
          const credit =
            item.entry_type === "CREDIT" ? parseFloat(item.amount || 0) : 0;
          const balanceClass =
            item.running_balance >= 0 ? "balance-positive" : "balance-negative";
          ...
          row.innerHTML = `
            ...
            <td class="${balanceClass}">${formatCurrency(
      ...
      showAlert(
        "alert-container",
        result.message || "فشل تحميل تاريخ الحساب",
        "error"
      );
    }
  } catch (error) {
